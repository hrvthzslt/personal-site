...
Just to be clear edit-compile-edit cycle is the act of editing ones glorious code, try to compile it, then editing again which hopefully mean all the previous compilation errors will be resolved. And the wheel turns, again and again...
...
I have an answers to this: You are not an IDE, you're just a text editor! And I have a question as well: Then what the hell is this?

![C with stock Vim](c-with-stock-vim.png)

Well, yes these are missing semicolons, but that is not the point. The point is that without any configuration, or plugin, compilation can be done from Vim, and issues can be jumped to and fixed. This is automatically work with C for example, so I will deduct without doing any investigation that Vim was indeed mostly written with Vim, facilitating the edit-compile-edit cycle.
...
The backbone of the edit-compile-edit cycle in Vim is the `:make` command. It runs the program defined in the `makeprg` option. It's default value is **make**, that can be called with the `:make` command which will run the default **make target**. This is a lot of _make_, I hope it _makes_ sense (ha!)

The output of the `:make` command will populate the `quickfix` list with any errors or warnings that the compiler outputs. You can then navigate through these issues using commands like `:cnext` and `:cprev`, like a ninja you are.
...
The original reason for the "Minimalist CLI Development Environment" was to create an environment with low footprint, but tailored to my convenience. This functionality has another reason for existing: Distraction free programming. After dipping my toes into C programing, I had to realize, that I need to go back and spend some quality time on learning. So I pick up this setup, a book and a coffee, and just roll.

![Blur - Coffee and Thinkpad](blur-coffe-and-thinkpad.png)
