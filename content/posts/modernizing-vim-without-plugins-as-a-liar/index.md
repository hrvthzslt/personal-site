...
The main commands in **Vim** for file and code navigation that I choose to use are `:find` and `:vimgrep`. `:find` is really really good, but it is slow if there are a lot of files in the project. You can offset that problem with creative usage of wildcards, and file previews at any time with `C-d`.

=gif= file previews

`:vimgrep` is also slow. The reason why I like it that it is available from Vim 7.0, that is the oldest version that I need compatibility with. Also it is independent from a system command, so the lack of a `grep` command or the fact that it has multiple implementations with different flags won't cause any problems.
...
There is an elegant way to integrate **fzf** with Vim, I was concerend about portability so I solved the problem at first with some horrible Vimscript. I'm only sharing the important part.

And for the record, at this point, I'm not even ashamed that I not come up with this. It runs a shell command which finds files excluding the `.git` directory, and then pipes the output to **fzf**. The result is written to a temporary file, which is read back into **Vim**, and then the first line is opened.
...
This is only just a segment, for demonstrating that I was not really contempt with this solution. And it did break another compatibility, up until this time, this config was compatible with **Neovim** as well, and I was not up for figuring out how to fix that.

=gif= fzf the stupid way

## fzf like a Normal Person

The really cool thing that **fzf** packaged with a vim plugin, so yes, you cannot trust me there is a plugin, but it is not handled by a plugin manager. It provides a command `:FZF`, I'm sure you can figure out its purpose.

There is still two thing that needs to be handled: Setting up a command for **fzf** to ignore the `.git` directory, and adding the plugin path to the `runtimepath`. That is path for runtime files, _the more you know_...
...
`rtp` stand for `runtimepath`, there are two paths, the first is the plugin path in Debian (and I guess on other distros as well), and the second is the plugin path for Homebrew on Mac. I do not check their existence, life is unfair, grow up.
...
The `SearchFiles` function checks if the `:FZF` command exists, and if the `modern_tools` variable is set to true. I introduced an additonal variable to provide the ability to disable this whole craziness.
...
I use it for searching for word under the cursor in selected file types and for general search, lets inspect the latter:
...
In the end, the `:grep` command will be typed and the cursor will be positioned and wait for our search term. The TUI fall apart after searches, so I added `redraw!` to fix that, and indeed it fixes that.
